...
const DEFAULTS = { notifications: false, osNotifications: false, overPlanThreshold: 12, restBudgetThreshold: 45 }
...
function paceFor(window) {
  if (!window?.reset_at || window.reset_state === 'refreshing') return null
  const reset = new Date(window.reset_at); if (Number.isNaN(reset.valueOf())) return null
  const start = new Date(reset.valueOf() - WEEK_MS); const now = new Date(Math.min(Math.max(Date.now(), start.valueOf()), reset.valueOf()))
  const target = clamp(100 * (now - start) / WEEK_MS); const delta = window.used_percent - target
...
function UsageView({ ctx }) {
  ...
  const response = query.data; const data = response?.data; const pace = useMemo(() => paceFor(data?.week), [data?.week]); const message = paceCopy(pace)
  ...
  return jsxs('section', { className: 'flex h-full flex-col gap-3 overflow-auto p-3 text-sm', children: [
    ...
    jsxs('div', { className: 'flex flex-col gap-2 rounded-md border border-(--ui-stroke-secondary) p-3', children: [jsx('div', { className: 'font-medium text-foreground', children: 'Einstellungen' }), jsxs('label', { className: 'flex items-center gap-2 text-[0.75rem] text-(--ui-text-secondary)', children: [jsx('input', { type: 'checkbox', checked: settings.notifications, onChange: e => update({ notifications: e.target.checked }) }), 'Lokale Hinweise aktivieren'] }), settings.notifications ? jsxs('label', { className: 'flex items-center gap-2 text-[0.75rem] text-(--ui-text-secondary)', children: [jsx('input', { type: 'checkbox', checked: settings.osNotifications, onChange: e => update({ osNotifications: e.target.checked }) }), 'OS-Benachrichtigungen aktivieren'] }) : null] }),
