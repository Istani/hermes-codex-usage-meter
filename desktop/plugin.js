...
const DEFAULTS = { mode: 'workweek', notifications: false, osNotifications: false, keepLocalQuotaOnRemote: false, overPlanThreshold: 12, restBudgetThreshold: 45 }

// ctx.rest() is profile-aware and follows the focused chat. A plugin cannot
// retarget this private REST namespace to the desktop-local backend. We resolve
// the focused owner against its exact route so unrelated remote profiles never
// make a This-device chat look remote.
async function readUsage(ctx) {
  ...
  try {
    ...
    if (owner) route = routes.find(item => item.connectionId === owner.connectionId && (item.profile === owner.profile || item.targetProfile === owner.profile)) || null
  } catch {}
  const response = await ctx.rest('/usage', { timeoutMs: 20000 })
  return { ...response, route_mode: route?.mode || host.state.gateway?.get?.()?.mode || 'unknown' }
}

async function refreshUsage(ctx) {
  const response = await ctx.rest('/refresh', { method: 'POST', timeoutMs: 20000 })
  return { ...response, route_mode: host.state.gateway?.get?.()?.mode || 'unknown' }
}

function snapshotResponse(liveResponse, snapshot, settings) {
  if (liveResponse?.route_mode === 'remote' && settings.keepLocalQuotaOnRemote) {
    return snapshot ? { ...snapshot, state: 'snapshot', stale: true, message: 'Lokaler Snapshot – Remote-Gateway wird ignoriert.' } : { state: 'unavailable', data: null, stale: false, message: 'Kein lokaler Snapshot vorhanden. Öffne das Plugin einmal bei „This device“.' }
  }
  return liveResponse
}
...
function useLocalSnapshot(ctx, liveResponse) { const [snapshot, setSnapshot] = useState(null); useEffect(() => { let alive = true; Promise.resolve(ctx.storage.get('localQuotaSnapshot')).then(saved => { if (alive && saved?.data) setSnapshot(saved) }).catch(() => {}); return () => { alive = false } }, [ctx]); useEffect(() => { if (liveResponse?.state !== 'ok' || liveResponse?.route_mode !== 'local' || !liveResponse?.local_source || !liveResponse?.data) return; const next = { data: liveResponse.data, source_updated_at: liveResponse.source_updated_at, captured_at: new Date().toISOString() }; setSnapshot(next); Promise.resolve(ctx.storage.set('localQuotaSnapshot', next)).catch(() => {}) }, [ctx, liveResponse]); return snapshot }

function UsageView({ ctx }) {
  const [settings, update] = useSettings(ctx); const client = useQueryClient()
  const query = useQuery({ queryKey: [ID, 'usage'], queryFn: () => readUsage(ctx), enabled: true, refetchInterval: () => document.visibilityState === 'visible' ? 60000 : 300000, retry: 1, staleTime: 45000, refetchOnWindowFocus: true })
  const refresh = useMutation({ mutationFn: () => refreshUsage(ctx), onSuccess: data => client.setQueryData([ID, 'usage'], data) })
  const liveResponse = query.data; const snapshot = useLocalSnapshot(ctx, liveResponse); const response = snapshotResponse(liveResponse, snapshot, settings); const data = response?.data; const pace = useMemo(() => paceFor(data?.week, settings.mode), [data?.week, settings.mode]); const message = paceCopy(pace)
  ...
  const stateText = query.isLoading ? 'Lade lokale Codex-Limits …' : response?.message || (query.isError ? 'Abruf fehlgeschlagen.' : 'Nicht verfügbar'); const sourceLabel = response?.state === 'snapshot' ? 'Lokaler Snapshot (keine Remote-Abfrage)' : liveResponse?.route_mode === 'remote' ? 'Remote-Gateway-Daten' : 'Lokale Datenquelle'
  return jsxs('section', { className: 'flex h-full flex-col gap-3 overflow-auto p-3 text-sm', children: [
    ...
    jsxs('div', { className: 'flex flex-col gap-2 rounded-md border border-(--ui-stroke-secondary) p-3', children: [jsx('div', { className: 'font-medium text-foreground', children: 'Einstellungen' }), jsxs('label', { className: 'flex items-center justify-between gap-3 text-[0.75rem] text-(--ui-text-secondary)', children: [jsx('span', { children: 'Verteilungsmodus' }), jsx('select', { className: 'rounded border border-(--ui-stroke-secondary) bg-transparent px-1 py-0.5 text-[0.75rem] text-foreground', value: settings.mode, onChange: e => update({ mode: e.target.value }), children: [jsx('option', { value: 'workweek', children: 'Arbeitswoche' }), jsx('option', { value: 'calendar', children: 'Kalenderwoche' })] })] }), jsxs('label', { className: 'flex items-center gap-2 text-[0.75rem] text-(--ui-text-secondary)', children: [jsx('input', { type: 'checkbox', checked: settings.notifications, onChange: e => update({ notifications: e.target.checked }) }), 'Lokale Hinweise aktivieren'] }), jsxs('label', { className: 'flex items-start gap-2 text-[0.75rem] text-(--ui-text-secondary)', children: [jsx('input', { type: 'checkbox', checked: settings.keepLocalQuotaOnRemote, onChange: e => update({ keepLocalQuotaOnRemote: e.target.checked }) }), jsx('span', { children: 'Bei Remote-Gateways letzten lokalen Snapshot anzeigen' })] }), settings.notifications ? jsxs('label', { className: 'flex items-center gap-2 text-[0.75rem] text-(--ui-text-secondary)', children: [jsx('input', { type: 'checkbox', checked: settings.osNotifications, onChange: e => update({ osNotifications: e.target.checked }) }), 'OS-Benachrichtigungen aktivieren'] }) : null] }),
    ...
    jsx('p', { className: 'text-[0.6875rem] text-(--ui-text-quaternary)', children: response?.source_updated_at ? `Zuletzt erfolgreich aktualisiert: ${formatTime(response.source_updated_at)} · ${sourceLabel}` : sourceLabel })
  ] })
}

function Status({ ctx }) { const [settings] = useSettings(ctx); const query = useQuery({ queryKey: [ID, 'usage'], queryFn: () => readUsage(ctx), enabled: true, refetchInterval: () => document.visibilityState === 'visible' ? 60000 : 300000, retry: 1, staleTime: 45000 }); const liveResponse = query.data; const snapshot = useLocalSnapshot(ctx, liveResponse); const response = snapshotResponse(liveResponse, snapshot, settings); const data = response?.data; const warning = response?.stale || query.isError || !data; const label = warning ? (response?.message || 'Lokale Codex-Limits nicht verfügbar') : `5 h: ${data.five_hour?.remaining_percent ?? '—'} % übrig · Woche: ${data.week?.remaining_percent ?? '—'} % übrig`; return jsx(Tip, { label, children: jsxs('button', { type: 'button', className: 'inline-flex h-full items-center gap-1.5 px-1.5 text-[0.6875rem] text-(--ui-text-tertiary) hover:text-foreground', onClick: () => { haptic('tap'); host.openWorkspace(`${ID}.detail`, { title: 'Codex-Verbrauch', render: () => jsx(UsageView, { ctx }) }) }, children: [jsx('span', { className: 'text-(--ui-text-quaternary)', children: response?.state === 'snapshot' ? 'Codex lokal*' : 'Codex' }), jsx(Meter, { label: '5 h', window: data?.five_hour, compact: true }), jsx(Meter, { label: 'Woche', window: data?.week, compact: true }), warning ? jsx('span', { className: 'text-(--ui-yellow)', children: '!' }) : null] }) }) }
